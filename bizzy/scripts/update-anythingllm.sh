...
# Script to update AnythingLLM to the latest version
...
echo -e "${GREEN}Starting AnythingLLM update process...${NC}"

# Navigate to the AnythingLLM directory
cd "$(dirname "$0")/../core/anythingllm" || {
    echo -e "${RED}Error: Could not navigate to AnythingLLM directory${NC}"
    exit 1
}
...
echo -e "${YELLOW}Current AnythingLLM version: ${CURRENT_VERSION}${NC}"
...
echo -e "${YELLOW}Latest AnythingLLM version: ${LATEST_TAG}${NC}"
